get.res<-function (out, type = "pearson", batch = NULL) 
{
  ...
  mu.full <- exp(out$a %*% t(out$W) + out$gmean + out$Mb[, 
                                                         apply(out$M, 1, which)])
  Wa.mean = out$a %*% as.matrix(colMeans(out$W))
  idx.gene <- abs(log(rowMeans(Y + 1)/rowMeans(mu + 1))) > 
    log(5)
  if (sum(idx.gene) > 1) 
    mu[idx.gene, ] <- rowMeans(Y[idx.gene, ])
  if (sum(idx.gene) == 1) 
    mu[idx.gene, ] <- mean(Y[idx.gene, ])
  idx.gene <- abs(log(rowMeans(Y + 1)/rowMeans(mu.full + 1))) > 
    log(5)
  ...
  if (type == "pearson") {
    if (is.null(dim(out$psi))) 
      dev <- (Y - mu * (1 - out$pi0))/sqrt(mu * (1 - out$pi0) * 
                                             (1 + mu * (out$psi.DE + out$pi0)))
    if (!is.null(dim(out$psi))) 
      dev <- (Y - mu * (1 - out$pi0))/sqrt(mu * (1 - out$pi0) * 
                                             (1 + mu * (out$psi.DE[, batch] + out$pi0)))
  ...
  if (type == "quantile") {
    mu.noUV <- exp(out$Mb[, apply(out$M, 1, which)] + out$gmean + 
                     c(Wa.mean))
    idx.gene <- abs(log(rowMeans(Y + 1)/rowMeans(mu.noUV + 
                                                   1))) > log(5)
    if (sum(idx.gene) > 1) 
    ...
    if (is.null(dim(out$psi))) {
      a <- ZIM::pzinb(Y - 1, lambda = mu.full, k = 1/out$psi.DE, 
                      omega = out$pi0)
      b <- ZIM::pzinb(Y, lambda = mu.full, k = 1/out$psi.DE, 
                      omega = out$pi0)
      a[a > 0.999] <- 0.9985
      b[b > 0.999] <- 0.999
      ...
      for (i in 1:ncol(Y)) {
        ...
        dev[, i] <- ZIM::qzinb(p[, i], lambda = mu.noUV[, 
                                                        i], k = 1/out$psi.DE, omega = avg.pi0)
      }
    }
    if (!is.null(dim(out$psi))) {
      a <- ZIM::pzinb(Y - 1, lambda = mu.full, k = 1/out$psi.DE[, 
      ...
      for (i in 1:ncol(Y)) {
        p[, i] <- runif(nrow(Y), a[, i], b[, i])
        dev[, i] <- ZIM::qzinb(p[, i], lambda = mu.noUV[, 
                                                        i], k = 1/out$psi.DE[, batch[i]], omega = avg.pi0)
      }
    }
  }
  dev
